    ...
    def parser(self, src, dst):
        ...
        try:
            ...
            if (jsonData['status'] != 'ok'):
                raise Exception('unexcepted status \'%s\', status should be \'ok\'' % jsonData['status'])

                return
            mdData = self.json2md(jsonData)
            self.saveData(dst, mdData)
        except Exception as e:
            print('Error: %s [filePath=%s]' % (e, src))
        finally:
            jsonFile.close()
        ...
        for parent, dirnames, filenames in os.walk(self.srcDir):
            for filename in filenames:
                ...
                try:
                    jsonData = eval(jsonFile.read())
                    if (jsonData['status'] != 'ok'):
                        print('Warning: skip error json [filePath=%s]' % filePath)
                        continue
                    mdData = self.json2md(jsonData)
