...
class ExerciseChecker:
    ...
    def checkJsonData(self, jsonData):
        ...
        if typeStr in ['single_answer', 'true_false']:
            ...
            if jsonData['answer'].strip() == '解释' or not jsonData['answer'].strip():  # 这是一个特殊处理，因为很多题目的答案只有"解释"
                if self.ignoreWarning:
                    jsonData['status'] = 'error'
                else:
                    raise InvailExerciseException('invail json [type=%s] [answer=%s]' % (typeStr, jsonData['answer'].strip()))
...
class Md2jsonParser:
        ...
        for parent, dirnames, filenames in os.walk(self.srcDir):
            for filename in filenames:
                ...
                try:
                    ...
                    print('Warning: %s [filePath=%s]' % (e, filePath))
                    self.warningCount += 1
                except Exception as e:
                    print('Error: %s [filePath=%s]' % (e, filePath))
                    self.failureCount += 1
                else:
                    self.successCount += 1
    ...
    def mdParseType(self, mdData, mdContent):
        ...
        typeNo = int(line)
        if typeNo not in list(range(1, 6)):
    ...
    def mdParseExtraInfo(self, mdData, mdContent):
        ...
        while line.strip().startswith('>'):
            regx = '>\s*([^：]+)：(\S+)。?'  # 注意左边的字符串中冒号和句号是在中文状态下输入的
            match = re.search(regx, line)
            if match:
                ...
                if infoKey == '知识点':
                    ...
                    mdData['knowledge'] = [infoValue.strip('。')]  # 这里的句号在中文状态输入
                elif infoKey == '出处':
